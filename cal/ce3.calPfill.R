ce3.calPfill <- function(ccc){
    ...
    if(a$cmscok == "opK1" |a$cmscok == "opK2"|a$cmscok == "opK3"){
        ...
        if(length(icdga > 0)){
            ...
            ixa001    <- which(pdriftfill < cdgax001border)
            ixa01     <- which((pdriftfill < cdgax01border) & (pdriftfill > cdgax001border))
            ixa1      <- which((pdriftfill < cdgax1border ) & (pdriftfill > cdgax01border))

            if(length(ixa1) > 0){
                pdriftfill[ixa1]       <- pdriftfill[ixa1]   -  poax1[ixa1]
                pfill[ixa1]            <- pfill[ixa1]        -  poax1[ixa1]
                pfilloffset[ixa1]      <- poax1[ixa1]
            }

            if(length(ixa01) > 0){
                pdriftfill[ixa01]      <- pdriftfill[ixa01] -  poax01[ixa01]
                pfill[ixa01]           <- pfill[ixa01]      -  poax01[ixa01]
                pfilloffset[ixa01]     <- poax01[ixa01]

            }
            if(length(ixa001) > 0){
                pdriftfill[ixa001]     <- pdriftfill[ixa001]-  poax001[ixa001]
                pfill[ixa001]          <- pfill[ixa001]     -  poax001[ixa001]
                pfilloffset[ixa001]    <- poax001[ixa001]
            }
        ...
        if(length(icdgb > 0)){
            ...
            ixb01  <- which((pdriftfill < cdgbx01border) & (pdriftfill > cdgbx001border))
            ixb1   <- which((pdriftfill < cdgbx1border)  & (pdriftfill > cdgbx01border))

            if(length(ixb1)>0){
                pdriftfill[ixb1]     <- pdriftfill[ixb1]   -  pobx1[ixb1]
                pfill[ixb1]          <- pfill[ixb1]        -  pobx1[ixb1]
                pfilloffset[ixb1]    <- pobx1[ixb1]

            }

            if(length(ixb01)>0){
                pdriftfill[ixb01]    <- pdriftfill[ixb01]  -  pobx01[ixb01]
                pfill[ixb01]         <- pfill[ixb01]       -  pobx01[ixb01]
                pfilloffset[ixb01]   <- pobx01[ixb01]
            }
        ...
        ## Bei sehr kleinen Fülldrücken  können
        ## Offsetschwankungen ect. problematisch
        ## werden; hier wird dann nicht korrigiert
        ## icorrdp <- which(pfill > 0)#cdgax001border)
        ## pdriftfill[icorrdp] <- pdriftfill[icorrdp]  - dpC[icorrdp]
        ## pfill[icorrdp]      <- pfill[icorrdp] - dpC[icorrdp]
    ...
    if(a$cmscok == "opK4"){
        ...
        if(length(icdga) > 0){
            ...
            msg      <- paste(msg,"assume cdga for points:", toString(icdga))
            ixa001    <- which(pfill < cdgax001border)
